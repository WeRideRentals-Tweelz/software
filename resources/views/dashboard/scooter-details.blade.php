...
@section('content')
<!-- Back button -->
<div class="col-xs-1" style="position: fixed">
	<a href="{{ url('/home/scooters') }}" class="btn btn-info btn-lg"><span class="fa fa-undo fa-2x"></span>Back</a>
</div>
					...
					<h2>Bookings History</h2>
						...
						<table class="table table-striped table-condensed">
							...
							<tbody>
								...
								<tr>
									<td>{{ $booking->id }}</td>
									<td><a href="{{ url('/profile/'.$booking->driver->user->id) }}">{{ $booking->user->name }}</a></td>
									<td>{{ $booking->pick_up_date }}</td>
									<td>{{ $booking->drop_off_date }}</td>
									<td>{{ $booking->drop_off_date - $booking->pick_up_date }}</td>
								</tr>
								@endforeach
							</tbody>
						</table>
